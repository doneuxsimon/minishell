...
void	remove_var(char **str, int i, t_var **var)
{
	t_var *tmp;
	int verif;
	...
	if ((*var)->before != NULL)
		(*var)->before->next = (*var)->next ;
	if ((*var)->next != NULL)
		*var = (*var)->next;
	else if ((*var)->before != NULL)
		*var = (*var)->before;
	else
		verif = 1;
	free(tmp->name);
...
void	ft_unset(char **str, t_var **var, int *i, int len)
{
	int verif;
...
//unset value puis utiliser unset pour supprimer les doublons dans les autres fonctions
